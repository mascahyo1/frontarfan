<template>
    <div class="container my-5">
      <h1 class="text-center text-primary">Update Showcase</h1>
      ...
      <form @submit.prevent="updateShowcaseRequest">
    ...
    <button type="submit" value="submit" class="btn btn-primary rounded-pill px-4 py-2" style="background-color: #007bff; border-color: #007bff;">Update</button>
  ...
  middleware: ['authenticated'],
    ...
    methods: {
      getData() {
        this.$axios.$get('/showcases/' + this.$route.params.id + "?populate=*")
          .then((response) => {
            console.log(this)
            this.form = response.data.attributes
            this.imageUrl = this.$imageurl + response.data.attributes.image.data.attributes.url
          })
          .catch((error) => {
            console.log(error)
          })
          ...
          try {
            if(this.selectedImage) {
              ...
              if(this.form.image.data) {
                this.form.image = this.form.image.data.attributes.id
              }
            }
            const response = await this.$axios.$request({
            url: '/showcases/' + this.$route.params.id,
            method: 'PUT',
